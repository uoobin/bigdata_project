...
f = open("prj_total_15.csv", "w", encoding="utf-8", newline='')
wr = csv.writer(f)
wr.writerow(["date", "word", "count"])

dt_index = pd.date_range(start='20150101', end='20151231')
...
data = open("prj_total_15.csv", "r", encoding="utf-8").read()
line = list(data.split('\n'))

spr = open("prj_15_spr.csv", "w", encoding="utf-8", newline='')
spr_wr = csv.writer(spr)
spr_wr.writerow(["word", "count"])
dt_spr = pd.date_range(start='20150301', end='20150531')
spr_list = dt_spr.strftime("%Y%m%d").tolist()

smr = open("prj_15_smr.csv", "w", encoding="utf-8", newline='')
smr_wr = csv.writer(smr)
smr_wr.writerow(["word", "count"])
dt_smr = pd.date_range(start='20150601', end='20150831')
smr_list = dt_smr.strftime("%Y%m%d").tolist()  

aut = open("prj_15_aut.csv", "w", encoding="utf-8", newline='')
aut_wr = csv.writer(aut)
aut_wr.writerow(["word", "count"])
dt_aut = pd.date_range(start='20150901', end='20151130')
aut_list = dt_aut.strftime("%Y%m%d").tolist()    

wtr = open("prj_15_wtr.csv", "w", encoding="utf-8", newline='')
wtr_wr = csv.writer(wtr)
wtr_wr.writerow(["word", "count"])
dt_wtr1 = pd.date_range(start='20150101', end='20150228')
dt_wtr2 = pd.date_range(start='20151201', end='20151231')
