...
f = open("prj_total_19.csv", "w", encoding="utf-8", newline='')
wr = csv.writer(f)
wr.writerow(["date", "word", "count"])

dt_index = pd.date_range(start='20190101', end=pd.datetime.today())
...
data = open("prj_total_19.csv", "r", encoding="utf-8").read()
line = list(data.split('\n'))

spr = open("prj_19_spr.csv", "w", encoding="utf-8", newline='')
spr_wr = csv.writer(spr)
spr_wr.writerow(["word", "count"])
dt_spr = pd.date_range(start='20190301', end='20190531')
spr_list = dt_spr.strftime("%Y%m%d").tolist()

smr = open("prj_19_smr.csv", "w", encoding="utf-8", newline='')
smr_wr = csv.writer(smr)
smr_wr.writerow(["word", "count"])
dt_smr = pd.date_range(start='20190601', end='20190831')
smr_list = dt_smr.strftime("%Y%m%d").tolist()  

aut = open("prj_19_aut.csv", "w", encoding="utf-8", newline='')
aut_wr = csv.writer(aut)
aut_wr.writerow(["word", "count"])
dt_aut = pd.date_range(start='20190901', end='20191130')
aut_list = dt_aut.strftime("%Y%m%d").tolist()    

wtr = open("prj_19_wtr.csv", "w", encoding="utf-8", newline='')
wtr_wr = csv.writer(wtr)
wtr_wr.writerow(["word", "count"])
dt_wtr1 = pd.date_range(start='20190101', end='20190228')
dt_wtr2 = pd.date_range(start='20191201', end=pd.datetime.today())
