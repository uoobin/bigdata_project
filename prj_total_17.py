...
f = open("prj_total_17.csv", "w", encoding="utf-8", newline='')
wr = csv.writer(f)
...
data = open("prj_total_17.csv", "r", encoding="utf-8").read()
line = list(data.split('\n'))

spr = open("prj_17_spr.csv", "w", encoding="utf-8", newline='')
spr_wr = csv.writer(spr)
spr_wr.writerow(["word", "count"])
dt_spr = pd.date_range(start='20170301', end='20170531')
spr_list = dt_spr.strftime("%Y%m%d").tolist()

smr = open("prj_17_smr.csv", "w", encoding="utf-8", newline='')
smr_wr = csv.writer(smr)
smr_wr.writerow(["word", "count"])
dt_smr = pd.date_range(start='20170601', end='20170831')
smr_list = dt_smr.strftime("%Y%m%d").tolist()  

aut = open("prj_17_aut.csv", "w", encoding="utf-8", newline='')
aut_wr = csv.writer(aut)
aut_wr.writerow(["word", "count"])
dt_aut = pd.date_range(start='20170901', end='20171130')
aut_list = dt_aut.strftime("%Y%m%d").tolist()    

wtr = open("prj_17_wtr.csv", "w", encoding="utf-8", newline='')
wtr_wr = csv.writer(wtr)
wtr_wr.writerow(["word", "count"])
dt_wtr1 = pd.date_range(start='20170101', end='20170228')
dt_wtr2 = pd.date_range(start='20171201', end='20171231')
